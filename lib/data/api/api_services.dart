...
class ApiServices {
    ...
    Future<RestaurantDetailResponse> getRestaurantDetail(String id) async{
      ...
      if (response.statusCode == 200) {
        RestaurantDetailResponse data = RestaurantDetailResponse.fromJson(jsonDecode(response.body));
        return data;
      } else {
        throw Exception('Failed to load restaurant list');
      }
    ...
    Future<RestaurantListResponse> getSearchResults(String searchText) async {
      final response = await http.get(Uri.parse("$_baseUrl/search?q=$searchText"));
